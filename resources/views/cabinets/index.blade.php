<x-app-layout>
    <div x-data="{
        createOpen: false,
        editOpen: false,
        editData: { id: null, nama: '', tahun: '', active: false },
    
        // Fungsi untuk membuka modal edit & isi datanya
        openEditModal(id, nama, tahun, active) {
            this.editData.id = id;
            this.editData.nama = nama;
            this.editData.tahun = tahun;
            this.editData.active = active == 1 ? true : false;
            this.editOpen = true;
        }
    }" class="min-h-screen">
        ...
        <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8 relative">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 class="font-bold text-2xl text-gray-800 leading-tight">Manajemen Kabinet</h2>
                    <p class="text-sm text-gray-500">Atur Kabinet aktif dan arsip kepengurusan</p>
                </div>
            ...
            @if (session('success'))
                <div x-data="{ show: true }" x-show="show"
                    class="bg-green-50 border-l-4 border-green-500 p-4 rounded shadow-sm flex justify-between items-center mb-6">
                            ...
                            <div
                                ...
                                <div>
                                    <div class="flex items-center gap-3 mb-2">
                                        <span
                                            class="bg-blue-500 bg-opacity-50 border border-blue-400 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider flex items-center"><span
                                                class="w-2 h-2 bg-green-400 rounded-full mr-2 animate-pulse"></span>Sedang
                                            Menjabat</span>
                                        <span class="text-blue-200 font-medium text-sm flex items-center"><svg
                                ...
                                <div
                                    ...
                                    <button
                                        @click="openEditModal({{ $c->id }}, '{{ $c->nama_kabinet }}', '{{ $c->tahun_periode }}', {{ $c->is_active }})"
        ...
        <div x-show="createOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen px-4 text-center sm:block sm:p-0">
                <div x-show="createOpen" x-transition.opacity
                    class="fixed inset-0 bg-gray-900 bg-opacity-75 backdrop-blur-sm" @click="createOpen = false"></div>
                ...
                <div x-show="createOpen" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-2xl font-bold text-gray-900">Tambah Kabinet Baru</h3>
                    ...
                    <form action="{{ route('cabinets.store') }}" method="POST">
                        @csrf
                        <div class="space-y-5">
                            <div><label class="block text-sm font-bold text-gray-700 mb-1">Nama Kabinet</label><input
                                    type="text" name="nama_kabinet" required
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                    placeholder="Contoh: Kabinet Sinergi"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-1">Tahun Periode</label><input
                                    type="text" name="tahun_periode" required
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                    placeholder="Contoh: 2024/2025"></div>
                            <div class="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-start">
                                <div class="flex items-center h-5"><input id="create_active" name="is_active"
                                        value="1" type="checkbox"
                                        class="focus:ring-blue-500 h-5 w-5 text-blue-600 border-gray-300 rounded cursor-pointer">
                                </div>
                                <div class="ml-3 text-sm"><label for="create_active"
                                        class="font-bold text-blue-800 cursor-pointer">Set sebagai Kabinet
                                        Aktif?</label>
                        ...
                        <div class="mt-8 flex justify-end gap-3">
                            <button type="button" @click="createOpen = false"
                                class="bg-white text-gray-700 px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 font-medium">Batal</button>
                            <button type="submit"
                                class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-bold shadow-md">Simpan
                                Data</button>
                        </div>
                    </form>
                </div>
        ...
        <div x-show="editOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen px-4 text-center sm:block sm:p-0">
                ...
                <div x-show="editOpen" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-2xl font-bold text-gray-900">Edit Data Kabinet</h3>
                    ...
                    <form method="POST" :action="'/cabinets/' + editData.id">
                        @csrf
                        @method('PUT')

                        <div class="space-y-5">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Nama Kabinet</label>
                                <input type="text" name="nama_kabinet" x-model="editData.nama" required
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Tahun Periode</label>
                                <input type="text" name="tahun_periode" x-model="editData.tahun" required
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>

                            <div class="bg-yellow-50 p-4 rounded-xl border border-yellow-100 flex items-start">
                                <div class="flex items-center h-5">
                                    <input id="edit_active" name="is_active" value="1" type="checkbox"
                                        x-model="editData.active"
                                        class="focus:ring-yellow-500 h-5 w-5 text-yellow-600 border-gray-300 rounded cursor-pointer">
                                </div>
                                <div class="ml-3 text-sm">
                                    <label for="edit_active" class="font-bold text-yellow-800 cursor-pointer">Status:
                                        Kabinet Aktif</label>
                                    <p class="text-yellow-600 mt-1 text-xs">Mencentang ini akan menonaktifkan kabinet
                                        lainnya.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 flex justify-end gap-3">
                            <button type="button" @click="editOpen = false"
                                class="bg-white text-gray-700 px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 font-medium">Batal</button>
                            <button type="submit"
                                class="bg-yellow-600 text-white px-6 py-2 rounded-lg hover:bg-yellow-700 font-bold shadow-md">Update
                                Data</button>
                        </div>
                    </form>
                </div>
